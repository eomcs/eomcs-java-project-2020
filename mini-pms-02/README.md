...
이번 훈련에서는 자바에서 제공하는 클래스를 사용하여 표준 출력 장치로 값을 출력하는 것을 연습한다. 표준 출력 장치는 보통 터미널(terminal)을 가리키는 말이다. 콘솔(console) 이라고도 부른다. 자바 프로그램을 터미널에서 실행한다면 그 출력은 터미널 창으로 나온다. 이클립스 IDE로 실행한다면 **Console** 창으로 출력된다.
