...
보통 자바 프로그램은 여러 개의 클래스로 구성되고, 그 중 한 개의 클래스에 `main()` 메서드가 있다.
자바 프로그램을 실행할 때는 바로 이 `main()` 메서드가 있는 클래스를 실행하는 것이다.
나머지 클래스는 `main()` 메서드의 실행 흐름에 따라 여러 단계에 걸쳐 사용되어진다.

이번 훈련에서는 `App` 클래스만 실행 클래스로 만들 것이다.
*프로젝트(App2.java)* 와 *작업(App3.java)* 데이터를 다루는 코드는 `App` 클래스 하나에 합칠 것이다. 
또한 사용자가 입력한 명령어에 따라 데이터를 처리하도록 기능을 변경할 것이다.
이를 통해 `main()` 메서드의 용도와 조건문, 반복문의 활용을 더 심도 깊게 이해해 보자.
...
### 3단계 - 사용자로부터 명령어를 입력 받는 것을 반복한다

사용자가로부터 반복적으로 명령어를 입력 받게 한다. 
명령어가 'exit'나 'quit'이면 반복을 종료한다.
...
회원 등록 명령('/member/add') 이나 회원 목록 조회 명령('/member/list')를 구분한다. 
