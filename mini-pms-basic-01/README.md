...
프로그래밍의 시작은 **프로젝트 폴더** 를 생성하는 것에서 시작한다.

`프로그래밍`은 **컴퓨터가 해야 할 일을 명령어로 작성하는 과정** 을 가르키는 말이다. 프로그래밍 중에 생성되는 파일을 두는 폴더가 프로젝트 폴더다. 프로젝트 폴더에는 명령어 파일(.java)만 두는 것이 아니라 명령어 실행에 필요한 설정 정보를 담은 파일(.xml, .properties)이나 그림 파일(.gif, .jpeg) 등을 둔다.

그런데 이들 파일을 한 폴더에 두면 관리하기가 불편하다. 그래서 보통 하위 폴더를 만들어 구분한다. 자바 프로그래밍 세계에서는 국가나 회사, 개발자에 상관없이 일관된 방식으로 구분하기 위해 **메이븐(Maven) 빌드 도구의 표준 디렉토리 구조** 를 사용한다.
...
프로젝트 폴더를 직접 위와 같이 구성하려면 번거롭고 불편하다. **Gradle(그레이들)** 을 사용하면 이런 일을 쉽게 처리할 수 있다. **그레이들** 은 프로젝트 폴더 구성에서 테스트, 배포 파일 생성 등 애플리케이션 생성의 전반적인 작업을 수행한다. 보통 이런 도구를 **프로젝트 빌드 도구** 라 부른다. 
